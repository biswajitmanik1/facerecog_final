...
export default function DemoSession() {
  ...
  const handleRecognize = useCallback(async (dataUrl) => {
    if (!isLiveActive) return
    try {
      const res = await apiFetch('/api/demo/recognize', { method: 'POST', body: JSON.stringify({ image: dataUrl }) })
      const data = await res.json()
      if (data.success && data.faces && data.faces.length > 0) {
        setLastResult(data.faces[0])
        ...
        <div className="px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate('/dashboard')} className="p-2 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors group">
            ...
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all ${isLiveActive ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-slate-100 border-slate-200'}`}>
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setIsLiveActive(!isLiveActive)}
              ...
              {isLiveActive ? <><Square className="w-5 h-5" /> Stop Demo</> : <><Play className="w-5 h-5" /> Start Demo</>}
            </button>
            <button onClick={() => navigate('/dashboard')}
              className="px-6 py-3 rounded-xl font-semibold bg-blue-50 hover:bg-blue-100 text-blue-600 border-2 border-blue-200 hover:border-blue-300 transition-all duration-300 flex items-center gap-3 shadow-sm">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8">
            ...
            <div className="bg-white rounded-2xl border-2 border-blue-200 p-6 shadow-lg">
              ...
              <div className="space-y-6">
                <div className={`p-4 rounded-xl border-2 transition-all ${lastResult?.match ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-slate-600 text-sm font-medium">Status</span>
                    <div className={`px-3 py-1 rounded-full text-xs font-semibold ${lastResult?.match ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                      {lastResult?.match ? 'MATCH FOUND' : 'NO MATCH'}
                    </div>
                  </div>
                  <p className="text-slate-800 font-bold">
                    {lastResult?.match ? `Identified: ${lastResult.match.name}` : 'No face recognized'}
                  </p>
                </div>
                ...
                <div className="p-4 rounded-xl bg-gradient-to-br from-slate-50 to-blue-50 border-2 border-slate-200">
                  ...
                  <ul className="text-slate-600 text-sm space-y-2">
                    {['Click "Start Demo" to begin face recognition', 'Position your face clearly in the camera view', 'Results will appear here in real-time', 'Click "Stop Demo" to end the session'].map((tip, i) => (
          ...
          {processedImage && (
            <div className="bg-white rounded-2xl border-2 border-cyan-200 p-6 shadow-lg">
              ...
              <p className="text-slate-600 text-sm mt-4 text-center">AI-processed image with face detection and recognition overlay</p>
            </div>
          )}
        </div>
      </main>
